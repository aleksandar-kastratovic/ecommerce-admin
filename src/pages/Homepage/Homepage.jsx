...
const Homepage = () => {
  ...
  const { data: productTopSellingB2C, isLoading: isLoadingProductTopSellingB2C } = useQuery(["productTopSellingB2C"], () => api.get(`${B2CproductItemTopSelling}`).then((response) => response?.payload));
  const { data: productActiveCountB2C, isLoading: isLoadingProductActiveCountB2C } = useQuery(["productActiveCountB2C"], () => api.get(`${B2CproductItemActiveCount}`).then((response) => response?.payload));
  const { data: productLowStockCountB2C, isLoading: isLoadingProductLowStockCountB2C } = useQuery(["productLowStockCountB2C"], () => api.get(`${B2CproductItemLowStockCount}`).then((response) => response?.payload));
  ...
  const { data: cusGuestOrderedPercentageB2C, isLoading: isLoadingCusGuestOrderedPercentageB2C } = useQuery(["cusGuestOrderedPercentageB2C"], () => api.get(`${B2CcustomersGuestOrderedPercentage}`).then((response) => response?.payload));
  const { data: cusNeverOrderedPercentageB2C, isLoading: isLoadingCusNeverOrderedPercentageB2C } = useQuery(["cusNeverOrderedPercentageB2C"], () => api.get(`${B2CcustomersNeverOrderedPercentage}`).then((response) => response?.payload));
  const { data: cusSingleOrderPercentageB2C, isLoading: isLoadingCusSingleOrderPercentageB2C } = useQuery(["cusSingleOrderPercentageB2C"], () => api.get(`${B2CcustomersSingleOrderPercentage}`).then((response) => response?.payload));
  ...
  const { data: productTopSellingB2B, isLoading: isLoadingProductTopSellingB2B } = useQuery(["productTopSellingB2B"], () => api.get(`${B2BproductItemTopSelling}`).then((response) => response?.payload));
  const { data: productActiveCountB2B, isLoading: isLoadingProductActiveCountB2B } = useQuery(["productActiveCountB2B"], () => api.get(`${B2BproductItemActiveCount}`).then((response) => response?.payload));
  const { data: productLowStockCountB2B, isLoading: isLoadingProductLowStockCountB2B } = useQuery(["productLowStockCountB2B"], () => api.get(`${B2BproductItemLowStockCount}`).then((response) => response?.payload));
  ...
  const { data: cusNeverOrderedPercentageB2B, isLoading: isLoadingCusNeverOrderedPercentageB2B } = useQuery(["cusNeverOrderedPercentageB2B"], () => api.get(`${B2BcustomersNeverOrderedPercentage}`).then((response) => response?.payload));
  const { data: cusSingleOrderPercentageB2B, isLoading: isLoadingCusSingleOrderPercentageB2B } = useQuery(["cusSingleOrderPercentageB2B"], () => api.get(`${B2BcustomersSingleOrderPercentage}`).then((response) => response?.payload));
  ...
  const onChangeSystem = (res) => {
    const selectedValue = res.target.value;
    setValueSystem(selectedValue);
    saveSelectedValueToLocalStorage(selectedValue);
  }
  ...
  return (
    <>
      ...
      {isLoadingSkeleton ?
        ...
        <>
          <InputSelect
            styleFormControl={{ width: "fit-content", ".MuiFormLabel-root": { fontSize: "0.875rem" } }}
            usePropName={false}
          ...
          {valueSystem === 'b2c' && (
            <>
              <Box sx={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "1rem", marginTop: "0.5rem", "@media (max-width: 1200px)": { gridTemplateColumns: "1fr" } }}>
                <B2CLifetimeRevenue
                  isLoadingLifetimeRevenueB2C={isLoadingLifetimeRevenueB2C}
                  lifetimeRevenueB2C={lifetimeRevenueB2C}
                />
                <B2CAverageRevenue
                  isLoadingAverageRevenueB2C={isLoadingAverageRevenueB2C}
                  averageRevenueB2C={averageRevenueB2C}
                />
                <B2CTotalCustomers
                  isLoadingTotalCustomersDataB2C={isLoadingTotalCustomersDataB2C}
                  totalCustomersDataB2C={totalCustomersDataB2C}
                />
                <B2CStatus
                  isLoadingStatusCountB2C={isLoadingStatusCountB2C}
                  statusCountB2C={statusCountB2C}
                />
              </Box>
              ...
              <Box sx={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "1rem", marginTop: "1rem" }}>
                <B2CChartTabs />
                <B2CRecentOverview
                  isLoadingRecentOverviewB2C={isLoadingRecentOverviewB2C}
                  recentOverviewB2C={recentOverviewB2C}
                />
              </Box>
          ...
          {valueSystem === 'b2b' && (
            <>
              <Box sx={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "1rem", marginTop: "0.5rem", "@media (max-width: 1200px)": { gridTemplateColumns: "1fr" } }}>
                <B2BLifetimeRevenue
                  isLoadinglifetimeRevenueB2B={isLoadinglifetimeRevenueB2B}
                  lifetimeRevenueB2B={lifetimeRevenueB2B}
                />
                <B2BAverageRevenue
                  isLoadingAverageRevenueB2B={isLoadingAverageRevenueB2B}
                  averageRevenueB2B={averageRevenueB2B}
                />
                <B2BTotalCustomers
                  isLoadingTotalCustomersDataB2B={isLoadingTotalCustomersDataB2B}
                  totalCustomersDataB2B={totalCustomersDataB2B}
                />
                <B2BStatus
                  isLoadingStatusCountB2B={isLoadingStatusCountB2B}
                  statusCountB2B={statusCountB2B}
                />
              </Box>
              ...
              <Box sx={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "1rem", marginTop: "1rem" }}>
                <B2BChartTabs />
                <B2BRecentOverview
                  recentOverviewB2B={recentOverviewB2B}
                  isLoadingRecentOverviewB2B={isLoadingRecentOverviewB2B}
                />
              </Box>

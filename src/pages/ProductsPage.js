...
const ProductsPage = () => {
	...
	const saveProductDetailsService = async (saveData) => {
		const data = await saveProductService({
			...
			b2b_is_visible    : saveData.b2b_is_visible,
			description       : saveData.description,
			category_ids      : saveData.category_ids,
			options           : saveData.options,
			variants          : saveData.variants,
			vat               : saveData.vat,
			b2b_price         : saveData.b2b_price,
			b2b_purchase_price: saveData.b2b_purchase_price,
			sku               : saveData.sku,
			gtin              : saveData.gtin,
		...
		<>
			<section id="roles-page" className="card">
				<div className="tabs-container">
					<Tabs
						...
						onTabChange={(activeTabKey) => {
							...
							checkActiveTab(activeTabKey)
						}}
					/>
					<button type="button" className="btn-control button-add" onClick={() => {
						addProduct()
					}}>Novi proizvod
					</button>
				</div>
				...
				{activeTab === tabsList[1].eventKey && (
					<ProductDetails
						saveProduct={(data) => {
							saveProductDetailsService(data)
						}}
						productData={productDetailsData}
						addProduct={productNew}
						removeProduct={(data) => {
							removeProductDetailsService(data)
						}}
					/>
				)}
			</section>

...
const PromotionsUpSellsDetails = () => {
  ...
  let slugField = newFields.map((field) => {
    if (field.prop_name === "system" && usid !== "new") {
      return { ...field, disabled: true };
    } else {
      return field;
    }
  }).filter((field) => !(field.prop_name === "slug" && usid === "new"));


  const handleData = async () => {
    setIsLoading(true);
    api.get(`${apiPath}/${usid}`)
      .then((response) => {
    ...
    api.post(apiPath, data)
      .then((response) => {
        ...
        if (oldId === null) {
          ...
          navigate(`/promotions-up-sells/${tId}`, { replace: true });
        }
  ...
  const panelHandleSelect = (field) => {
    ...
    navigate(`/promotions-up-sells/${id}?${queryString}`, { replace: true });
  }

  return <DetailsPage title={data?.id == null ? "Promocija" : data?.name} fields={fields} ready={[usid === "new" || data?.id]} selectedPanel={activeTab} panelHandleSelect={panelHandleSelect} />;
};

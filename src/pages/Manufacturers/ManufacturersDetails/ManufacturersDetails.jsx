...
import { getManufacturer, saveManufacturer } from "../services";
...
const ManufacturersDetails = () => {
  ...
  const handleData = async () => {
    try {
      setIsLoading(true);
      let response = await getManufacturer(user.access_token, mmid);
  ...
  const saveData = async () => {
    try {
      let response = await saveManufacturer(user.access_token, data);
      handleBack();
  ...
  return (
    <PageWrapper title="Detalji proizvođača" back={handleBack}>

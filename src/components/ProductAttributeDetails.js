...
const ProductAttributeDetails = ({ productAttributeData, saveProductAttribute, removeProductAttribute }) => {
  ...
  const [productAttributes, setProductAttributes] = useState(
    (productAttributeData && productAttributeData.attribute_value_name)
    ?
    productAttributeData.attribute_value_name : initState
  );
  ...
  return (
    <div className="add-role-modal">
      <div className="btn-group mb-4" role="group" aria-label="Basic example">
        <button
          disabled={ !nameIsValid || !productAttributesValid ||  haveProductAttributeDuplicate}
        ...
        <button
          ...
          onClick={() => confirm(["Da li ste sigurni?", ()=> removeProductAttributeHandler() ])}
        >

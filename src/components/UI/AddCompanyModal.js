...
const AddCompanyModal = ({ openModal, handleClose, saveCompany }) => {
    ...
    let { value: accountValue, valueChangeHandler: accountChangeHandler, reset: resetAccount } = useInput((value) => value);

    let { value: dayPeymentValue, valueChangeHandler: dayPeymentChangeHandler, reset: resetDayPeyment } = useInput((value) => value.trim() !== "");

    let { value: creditLimitValue, valueChangeHandler: creditLimitChangeHandler, reset: resetCreditLimit } = useInput((value) => value.trim() !== "");

    let { value: billingAddressValue, valueChangeHandler: billingAddressChangeHandler, reset: resetBillingAddress } = useInput((value) => value.trim() !== "");

    let { value: billingCityValue, valueChangeHandler: billingCityChangeHandler, reset: resetBillingCity } = useInput((value) => value.trim() !== "");

    let { value: billingStateValue, valueChangeHandler: billingStateChangeHandler, reset: resetBillingState } = useInput((value) => value.trim() !== "");

    let { value: billingZipValue, valueChangeHandler: billingZipChangeHandler, reset: resetBillingZip } = useInput((value) => value.trim() !== "");

    let { value: billingCountryValue, valueChangeHandler: billingCountryChangeHandler, reset: resetBillingCountry } = useInput((value) => value.trim() !== "");

    let { value: shippingAddressValue, valueChangeHandler: shippingAddressChangeHandler, reset: resetShippingAddress } = useInput((value) => value.trim() !== "");

    let { value: shippingCityValue, valueChangeHandler: shippingCityChangeHandler, reset: resetShippingCity } = useInput((value) => value.trim() !== "");

    let { value: shippingStateValue, valueChangeHandler: shippingStateChangeHandler, reset: resetShippingState } = useInput((value) => value.trim() !== "");

    let { value: shippingZipValue, valueChangeHandler: shippingZipChangeHandler, reset: resetShippingZip } = useInput((value) => value.trim() !== "");

    let { value: shippingCountryValue, valueChangeHandler: shippingCountryChangeHandler, reset: resetShippingCountry } = useInput((value) => value.trim() !== "");

    let { value: statusValue, valueChangeHandler: statusChangeHandler, reset: resetStatus } = useInput((value) => value);

    let { value: priceListValue, valueChangeHandler: priceListChangeHandler, reset: resetPriceList } = useInput((value) => value);

    let { value: transportValue, valueChangeHandler: transportChangeHandler, reset: resetTransport } = useInput((value) => value);

    let { value: rabatValue, valueChangeHandler: rabatChangeHandler, reset: resetRabat } = useInput((value) => value);

    let { value: nonInvoiceRabatValue, valueChangeHandler: nonInvoiceRabatChangeHandler, reset: resetNonInvoiceRabat } = useInput((value) => value);

    let { value: expectedDelayValue, valueChangeHandler: expectedDelayChangeHandler, reset: resetExpectedDelay } = useInput((value) => value);

    const submitHandler = () => {
        if (!companyIsValid || !tinIsValid || !registryNumberIsValid || !emailIsValid || !phoneIsValid) {
    ...
    return (
        <Modal show={openModal} onHide={handleClose} backdrop="static" keyboard={false} centered size="xl" scrollable={true} className="add-role-modal">
            ...
            <Modal.Footer>
                ...
                <button disabled={!companyIsValid || !tinIsValid || !registryNumberIsValid || !emailIsValid || !phoneIsValid} type="button" className="btn-control save-btn" onClick={submitHandler}>
                    Sačuvajte
                </button>
            </Modal.Footer>
        </Modal>

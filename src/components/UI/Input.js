...
const Input = (props) => {
    ...
    const inputElement = () => {
        ...
        switch(props.inputType) {
            case "input": 
                return <>
                    {props.text && <p htmlFor={inputId} className={ (props.text_class ? props.text_class : '') + " form-control-label" }>{props.text}</p>}
                    <input autoComplete={props.offAutoComplete ? "new-password" : ''} value={props.inputValue} onKeyDown={props.onInputKeyDown} onChange={props.onInputChange} onBlur={props.onInputBlur} id={inputId} type={props.type ? props.type : 'text'} className={props.class ? props.class : ''} disabled={props.disabled} />
                    {errorVisible && props.text !== undefined && <p className={ "error-text " + (props.errorClass ? props.errorClass : '')}> {props.hasInputError ? (props.text + ' ' + errorText) : ''} </p>}
            ...
            case "select-react":
                return <>
                    {props.text && !props.remove && <p htmlFor={inputId} className={ (props.text_class ? props.text_class : '') + " form-control-label" }>{props.text}</p>}
                    {props.text && props.remove && 
                        <div className='row row-m0 justify-content-between'>
                            <p htmlFor={inputId} className={ (props.text_class ? props.text_class : '') + " form-control-label label-with-remove" }>{props.text}</p>
                            <button onClick={props.onRemove} className="btn-control remove-options">Ukloni</button>
                        </div>
                    }
                    <Select

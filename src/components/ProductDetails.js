...
const ProductDetails = ({ saveProduct, productData, addProduct, removeProduct }) => {
	...
	const variationInit = { variantId: null, mainImg: null, images: [], variant_name: "", price: null, purchase_price: null, sku: "", gtin: "", locations: [], variant_combinations: [] }
	...
	useEffect(() => {
		if (productData && Object.keys(productData).length > 0 && categoriesLoaded && locationsLoaded) {
			...
			b2bPriceChangeHandler({ target: { value: (productData?.b2b_price ?? "") } })
			purchasePriceChangeHandler({ target: { value: (productData?.b2b_purchase_price ?? "") } })
										...
										return (
											<div className="row" key={index} onClick={() => {
												...
												<div className="col-xl-1 align-self-center">
													...
													}} className="btn-control remove-options">X
													</button>
												</div>
											</div>
										)

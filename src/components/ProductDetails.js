...
const ProductDetails = ({
  ...
  let {
    value: gtinValue,
    valueChangeHandler: gtinChangeHandler,
    reset: resetGTIN,
  } = useInput((value) => value);

  let {
    value: b2bPriceValue,
    valueChangeHandler: b2bPriceChangeHandler,
    reset: resetB2BPrice,
  } = useInput((value) => value);
  ...
  let {
    value: vatValue,
    valueChangeHandler: vatChangeHandler,
    reset: resetVAT,
  } = useInput((value) => value);

  const [b2bIsVisible, setB2BIsVisible] = useState(1);
  ...
  const [locationsMultiQuantityInitList, setLocationsMultiQuantityInitList] =
    useState([]);
  const [locationsQuantityList, setLocationsQuantityList] = useState({
    location_id: null,
    quantity: 0,
  });
  const [selectedInventoryOptions, setSelectedInventoryOptions] = useState(
    inventoryOptions[1].id ?? null
  );
  ...
  useEffect(() => {
    if (
      ...
      additionalNameChangeHandler({
        target: { value: productData?.additional_name ?? "" },
      });
      ...
      setCategoryIds(
        productData.category_ids
          ? JSON.parse(JSON.stringify(productData.category_ids))
          : []
      );
      ...
      setB2BIsVisible(productData?.b2b_is_visible ?? 1);
      skuChangeHandler({ target: { value: productData?.sku ?? "" } });
      gtinChangeHandler({ target: { value: productData?.gtin ?? "" } });
      b2bPriceChangeHandler({
        target: { value: productData?.b2b_price ?? "" },
      });
      purchasePriceChangeHandler({
        target: { value: productData?.b2b_purchase_price ?? "" },
      });
      vatChangeHandler({ target: { value: productData?.vat ?? "" } });
      if (productData.locations?.length > 0) {
        if (productData.locations[0].location_id === null) {
          setSelectedInventoryOptions(null);
          setLocationsQuantityList({
            location_id: null,
            quantity: productData.locations[0].quantity,
          });
        } else {
          ...
          data.push({
            ...
            mainImg:
              item.images.length > 0
                ? imageForId(productData.image_ids, item.images[0])
                : null,
          });
        });
  ...
  const resetForm = () => {
    ...
    setLocationsMultiQuantityList(
      JSON.parse(JSON.stringify(locationsMultiQuantityInitList))
    );
    setMainImg(null);
  ...
  const getAttributeValues = async (attributeId) => {
    const data = await attributeValuesListService(
  ...
  const updateVariationsOnOptionsChange = (optionsData) => {
    ...
    optionsData.map((item, index) => {
      if (
        index > indexWithAttributes + 1 &&
        indexWithAttributes > -1 &&
        ...
        if (index > indexWithAttributes) {
          data.map((elem) => {
            const combinedVariation = {
              ...updateVariationsCombinations(elem, value, item.data),
            };
            combinedVariation.position =
              combinedVariation.position + index2.toString() + index.toString();
            dataFiltered.push(combinedVariation);
          });
        } else {
          let optItm = { ...variationInit };
          optItm.price = b2bPriceValue;
          optItm.purchase_price = purchasePriceValue;
          optItm.variant_combinations = [value];
          optItm.position = index2.toString() + index.toString();
          optItm.variant_name =
            nameValue + "-" + getNameForId(item.data, value);
  ...
  const updateVariationsCombinations = (item, itemForMerge, data) => {
    let mergedItem = { ...item };
    mergedItem.variant_combinations = [
      ...mergedItem.variant_combinations,
      itemForMerge,
    ];
    mergedItem.variant_name =
      mergedItem.variant_name + "-" + getNameForId(data, itemForMerge);
    return mergedItem;
  };
    ...
    variationsList.map((item) => {
      if (item.variant_combinations.length > 0) {
        variantsForSave.push({
          ...
          b2b_purchase_price: +item.purchase_price,
          sku: item.sku,
          vat: +item.vat,
          gtin: item.gtin,
  ...
  const submitHandler = () => {
    ...
    if (
      selectedProductAttributes.length > 1 ||
      (selectedProductAttributes.length > 0 && selectedProductAttributes[0].id)
    ) {
      ...
      saveProduct({
        ...
        options: [],
        variants: [],
        b2b_price: b2bPriceValue,
        b2b_purchase_price: purchasePriceValue,
        vat: vatValue,
        sku: skuValue,
        gtin: gtinValue,
  ...
  const checkOptionValueIsSelected = (newVariations) => {
    ...
    newVariations.map((nv) => {
      for (let [index, item] of selectedProductAttributes.entries()) {
        if (
          item.data.some((elem) => elem.id === nv) &&
          !item.valueIds.includes(nv)
        ) {
  ...
  const changeVariant = (data) => {
    ...
    oldData.mainImg =
      oldData.images.length > 0
        ? imageForId(galleryIds, oldData.images[0])
        : null;
    oldData.gtin = data.gtin;
    ...
    if (oldData.variant_name.trim() === "") {
      ...
      data.variant_combinations.map((item, index) => {
        if (item) {
          oldData.variant_name =
            oldData.variant_name +
            "-" +
            getNameForId(selectedProductAttributes[index].data, item);
        }
      });
    }
    delete oldData["selectedProductAttributes"];
    const variationIndex = variationsList.findIndex(
      (elem) => elem.variant_name == oldData.variant_name
    );
  ...
  const addNewVariation = () => {
    let optItm = { ...variationInit };
    let selectedProductAttributesReset = JSON.parse(
      JSON.stringify(selectedProductAttributes)
    );
  ...
  const createNextSku = () => {
    ...
    if (lastSku.trim() !== "") {
      if (Number.isInteger(+lastSku)) {
        ...
        sku = lastSku.slice(
          0,
          lastSku.length - (variationsList.length - 1).toString().length
        );
    ...
    <div className="add-role-modal">
      <div className="btn-group mb-4" role="group" aria-label="Basic example">
        ...
        <button
          ...
          onClick={() => setB2BIsVisible(b2bIsVisible === 0 ? 1 : 0)}
        >
          {b2bIsVisible === 0 && (
            <>
        ...
        <button
          ...
          onClick={() =>
            confirm([
              "Da li ste sigurni?",
              () => removeProduct({ product_ids: [productId] }),
            ])
          }
              ...
              <div className="col-xl-12 details-wrapper">
                ...
                <div className="row row-m0">
                  <div className="col-12 p-0">
                    <p
                      htmlFor="dropdownTreeSelectCategory"
                      className="m-0 form-control-label required"
                    >
                      Kategorije
                    </p>
                    <DropdownTreeSelect
                    ...
                    <p className="error-text">
                      {categoryIds.length < 1 && categoryHasFocus
        ...
        {activeTab === initTab[2].eventKey && variationsList.length < 1 && (
          <div className="col-9 details-wrapper-spacing">
            <div className="row">
              <div className="col-xl-12 details-wrapper">
                <Input
                  inputValue={b2bPriceValue}
                  onInputChange={b2bPriceChangeHandler}
                  disabled={false}
                  inputType="input"
                  type="number"
                  class={"form-control input-style form-control-lg"}
                ...
                <Input
                  inputValue={vatValue}
                  onInputChange={vatChangeHandler}
                  disabled={false}
                  inputType="input"
                  type="number"
                  class={"form-control input-style form-control-lg"}
            ...
            <div className="row">
              <div className="col-xl-12 details-wrapper">
                <p className="seo-text">
                  Popunite polja za SEO optimizaciju da bi se Vaš proizvod bolje
                  pozicionirao na Google pretrazi.
                </p>
                ...
                <p className="info-text">
                  0 od 320 karaktera iskorišćeno. Meta opis treba optimalno da
                  sadrži između 150-160 znakova.
                </p>
                <Input
                  inputValue={nameValue}
                  onInputChange={nameChangeHandler}
                  ...
                  <div className="gallery-wrapper">
                    ...
                    {!mainImg && (
                      <div
                        className={
                          "no-img-container " +
                          (gallery?.length > 0 ? "add-more-img-container" : "")
                        }
                      >
              ...
              <div className="col-xl-12 details-wrapper">
                ...
                <div className="row row-m0">
                  ...
                  <div className="col-6 pe-0">
                    <Input
                      inputValue={gtinValue}
                      onInputChange={gtinChangeHandler}
                      disabled={false}
                      inputType="input"
                      type="text"
                      class={
                        "form-control input-style form-control-lg form-control-6"
                      }
                      text="Bar-kod"
                    />
                  </div>
          ...
          <div className="col-9 details-wrapper-spacing">
            <div className="row">
              <div className="col-xl-12 details-wrapper">
                <div className="products-list-holder">
                  <p className="seo-text">
                    Inventar možete videti u varijacijama.
                  </p>
                </div>
              </div>
            </div>
                    ...
                    <div key={index} className="row row-m0">
                      <div className="col-4 ps-0">
                        <Input
                          ...
                          isMulti={false}
                          handleChange={(ev) =>
                            productAttributesChangeHandler(ev, index)
                          }
                          onInputBlur={nameBlurHandler}
                          ...
                          type="text"
                          class={
                            "form-control input-style form-control-lg select-style form-control-4"
                          }
                          text={"Opcija " + (index + 1)}
                        />
                      </div>
                      <div className="col-8 pe-0">
                        <Input
                          ...
                          isMulti={true}
                          handleChange={(ev) =>
                            attributeValuesChangeHandler(ev, index)
                          }
                          onInputBlur={nameBlurHandler}
                          ...
                          type="text"
                          class={
                            "form-control input-style form-control-lg select-style form-control-8"
                          }
                          ...
                          onRemove={() =>
                            confirm([
                              "Da li ste sigurni?",
                              () => removeSelectedProductAttributes(index),
                            ])
                          }
                        />
                      </div>
                  ...
                  {variationsList.map(function (object, index) {
                    return (
                      <div
                        ...
                        onClick={() => {
                          setShowModal(true);
                          setSelectedVariationForEdit(variationsList[index]);
                        }}
                      >
                        ...
                        <div className="col-xl-1 align-self-center">
                          <button
                            ...
                            className="btn-control remove-options"
                          >
                            X
                          </button>
                        </div>
          ...
          <div className="col-9 details-wrapper-spacing">
            <div className="row">
              <div className="col-xl-12 details-wrapper">
                <div className="products-list-holder">
                  <p className="seo-text">
                    Odaberite opcije da bi ste imali varijacije.
                  </p>
                </div>
              </div>
            </div>

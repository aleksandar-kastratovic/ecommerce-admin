...
import { faCity, faMoneyCheckAlt, faTruck } from "@fortawesome/free-solid-svg-icons";
...
const CompanyDetails = ({ companyData, saveCompany, removeCompany }) => {
    ...
    let { value: tinValue, isValid: tinIsValid, hasError: tinHasError, valueChangeHandler: tinChangeHandler, inputBlurHandler: tinBlurHandler, reset: resetTin } = useInput((value) => value > 0);
    ...
    let { value: creditLimitValue, valueChangeHandler: creditLimitChangeHandler, reset: resetCreditLimit } = useInput((value) => value > 0);
    ...
    return (
        <div className="add-role-modal">
            <div className="btn-group mb-4" role="group" aria-label="Basic example">
                <button
                    disabled={!companyIsValid || !tinIsValid || !registryNumberIsValid || !emailIsValid || !phoneIsValid}
                ...
                <button type="button" className="btn-control btn btn-delete-details" onClick={() => confirm(["Da li ste sigurni?", () => removeCompanyHandler()])}>
